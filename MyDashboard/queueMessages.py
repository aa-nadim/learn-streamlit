...
if authentication_status:
    ...
    count_yesterday = run_query(
        "SELECT COUNT(*) FROM tbl_outbox WHERE msg_status = -128 AND DATE(creation_datetime) = (CURRENT_DATE - INTERVAL '1 day');"
    )
    yesterday = count_yesterday[0][0]

    count_today = run_query(
        "SELECT COUNT(*) FROM tbl_outbox WHERE msg_status = -128 AND DATE(creation_datetime) = CURRENT_DATE;"
    )
