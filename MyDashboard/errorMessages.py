...
if authentication_status:
    ...
    error_massage = run_query(
        "SELECT COUNT(*) AS count_error_massages FROM tbl_outbox WHERE (op_status <> 9000 AND msg_status = 0) AND DATE(creation_datetime) = CURRENT_DATE;"
    )
    error = error_massage[0][0]
